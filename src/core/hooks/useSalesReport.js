...
const buildInitialFilters = (initialFilters) =>
  sanitize({ ...BASE_FILTERS, ...initialFilters });

export default function useSalesReport(initialFilters = {}) {
  ...
  const fetchReport = useCallback(async (currentFilters) => {
    ...
    setError(null);
    try {
      const payload = await ReportsService.getSalesReport(currentFilters);
      setData(payload);
  ...
  const resetFilters = useCallback(() => {
    const reset = sanitize(BASE_FILTERS);
    setFilters(reset);
  ...
  const exportReport = useCallback(
    async (filename = "sales-report.csv") => {
      setExporting(true);
      try {
        const blob = await ReportsService.exportSalesReport(appliedFilters);
